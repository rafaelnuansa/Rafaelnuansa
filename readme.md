<!-- Banner -->
<p align="center">
  <a href="https://github.com/rafaelnuansa">
    <img src="https://readme-typing-svg.herokuapp.com?font=Monaco&size=30&duration=2500&pause=1000&color=38F7AE&center=true&vCenter=true&width=750&lines=Rafael+Nuansa;Fullstack+Web+Developer;Let's+Build+Something+Great!+🚀" alt="Typing SVG" />
  </a>
</p>


<p align="center">
  <img src="https://img.shields.io/badge/Web%20Developer-%F0%9F%92%BB-blue?style=for-the-badge&logoColor=white">
  <img src="https://img.shields.io/badge/Tech%20Enthusiast-%F0%9F%94%A5-purple?style=for-the-badge&color=7A3EFF">
</p>

---

### 👨‍💻 About Me

Hi! I’m **Rafael Nuansa**, a passionate **Web Developer** who loves building applications that are  
**clean, scalable, secure, and solve real-world problems**.

- 🔥 Currently working with: Laravel, Vue.js & REST APIs
- 🎯 Learning path: DevOps, System Design & Go (Golang)
- 🚀 Passionate about optimization & application security
- 📍 Indonesia

---

### 🛠️ Tech Stack

#### Backend
<p>
  <img src="https://skillicons.dev/icons?i=php,laravel,go,nodejs,mysql,postgres,mongodb,sqlite" height="42" />
</p>

#### Frontend & Mobile
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,vue,react,tailwind,bootstrap,flutter" height="42" />
</p>

#### Tools & DevOps
<p>
  <img src="https://skillicons.dev/icons?i=git,github,linux,nginx,docker,vscode,postman,notion" height="42" />
</p>

---

### 📌 Highlight Projects

| Project | Stack | Description |
|--------|------|-------------|
| 🏦 HRIS System | Laravel, MySQL | HR platform with payroll, KPI, promotion & position mutation |
| 🎗️ Donation Dashboard | Laravel | Analytics dashboard with filtering & export features |

> 🌟 More exciting projects are in progress…

---

### 📈 GitHub Stats & Analytics

<p align="center">
  <!-- GitHub Streak -->
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=rafaelnuansa&theme=tokyonight&hide_border=true" height="160" />
</p>


<p align="center">
  <!-- Github Trophies -->
  <img src="https://github-profile-trophy.vercel.app/?username=rafaelnuansa&theme=tokyonight&no-frame=true&no-bg=true&margin-w=10" />
</p>

<p align="center">
  <!-- Views Counter -->
  <img src="https://komarev.com/ghpvc/?username=rafaelnuansa&style=flat-square&color=blueviolet" />
</p>

---

### 🤝 Connect With Me

<p align="left">
  <a href="https://github.com/rafaelnuansa"><img src="https://skillicons.dev/icons?i=github" height="40"></a>
  <a href="mailto:rafaelnuanska@hotmail.com"><img src="https://skillicons.dev/icons?i=gmail" height="40"></a>
  <a href="https://linkedin.com/in/rafaelnuansa"><img src="https://skillicons.dev/icons?i=linkedin" height="40"></a>
  <a href="https://instagram.com/rafaelnuansa"><img src="https://skillicons.dev/icons?i=instagram" height="40"></a>
</p>

---

> _“Talk is cheap. Show me the code.” — Linus Torvalds_  
Thanks for visiting — feel free to ⭐ any repo you like! 🚀
